		...
		if period == 'year':
			...
			request._cr.execute(f"""
				SELECT to_char(p.create_date,'Month') as month,COUNT(p.id)
					...
					GROUP BY 1
			""")
			product_counts = {data['month']:data['count'] for data in request._cr.dictfetchall()}
			...
			request._cr.execute(f"""
				SELECT to_char(o.date_order,'Month') as month,COUNT(o.id)
					...
					GROUP BY 1
			""")
			order_counts = {data['month']:data['count'] for data in request._cr.dictfetchall()}
			...
			request._cr.execute(f"""
				SELECT to_char(c.create_date,'Month') as month,COUNT(c.id)
					...
					GROUP BY 1
			""")
			category_counts = {data['month']:data['count'] for data in request._cr.dictfetchall()}
			...
			request._cr.execute(f"""
				SELECT to_char(c.create_date,'Month') as month,COUNT(c.id)
					...
					GROUP BY 1
			""")
			customer_counts = {data['month']:data['count'] for data in request._cr.dictfetchall()}
