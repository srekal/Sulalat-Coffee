...
class WkFeed(models.Model):
	...
	def get_product_id(self,store_product_id,line_variant_ids,channel_id ,default_code=None,barcode=None):
		...
		if barcode:
			domain += [('barcode','=',barcode)]
		product_id = None
		match = self._context.get('variant_mappings').get(channel_id.id,{}).get(line_variant_ids)
		if match:
	...
	def contextualize_mappings(self,type,channel_ids=False):
		...
		if type == 'category':
			...
			for rec in self.env['channel.product.mappings'].search_read(
				[('channel_id','in',channel_ids)],
				['id','store_variant_id','channel_id'],
			):
				variant_mappings.setdefault(rec.get('channel_id')[0],{})[rec.get('store_variant_id')] = rec.get('id')
